...
import { useSession, useApi, useWebsocket, useToast, ToastType, useLanguage, useEvent, useOrder } from '@components'
...
export const OrderDetails = (props) => {
  ...
  const [showPickupComplete, setShowPickupComplete] = useState(false)
  const [showDeliveryComplete, setShowDeliveryComplete] = useState(false)

  const [imageuploadLloading, setImageuploadLoading] = useState(false)
  /**
  ...
  const handleDeliveryReceipt = async (url) => {
    try {
      setOrderState({ ...orderState, loading: true })
      const { content: { result, error } } = await ordering.setAccessToken(token).orders(orderState.order?.id ?? orderId).save({ delivery_receipt: url, status: 11 })
      ...
      if (!error) {
        ...
        console.log('ordertotalearns')
        console.log(result?.ordertotalearns)
        await AsyncStorage.setItem('deliveryTip', result?.ordertotalearns.toString());
  ...
  const handleUpdateReceipt = async (url) => {
    try {
      setOrderState({ ...orderState, loading: true })
      const { content: { result, error } } = await ordering.setAccessToken(token).orders(orderState.order?.id ?? orderId).save({ pickup_receipt: url })
      ...
      if (!error) {
        ...
        props?.handleopendetails && props?.handleopendetails()
        setImageuploadLoading(false)
  ...
  const handleUpdateOrderData = async (delaymin, comments) => {
    try {
      setOrderState({ ...orderState, loading: true })
      const { content: { result, error } } = await ordering.setAccessToken(token).orders(orderState.order?.id ?? orderId).save({ delivered_in: delaymin, delay_reason: comments })
    ...
    <>
      {UIComponent && (
        <UIComponent
          ...
          showPickupComplete={showPickupComplete}
          showDeliveryComplete={showDeliveryComplete}
          handleDeliveryReceipt={handleDeliveryReceipt}
          imageuploadLoading={imageuploadLloading}
        />
      )}
    </>
  )
