...
const OrdersOptionUI = (props: OrdersOptionParams) => {
  ...
  const showTagsList = !props.isDriverApp && currentTabSelected !== 'logisticOrders'
  ...
  return (
    <>
      ...
      <View style={[styles.header, {
        ...
        <TouchableOpacity onPress={() => goToBack()} style={[styles.btnBackArrow, { alignItems: 'center' }]}>
          ...
          <FeatherIcon
            ...
            color='#000000'
          />
          {/* <OIcon src={theme.images.general.arrow_left} style={{

          }} color='#1B3E70' /> */}
        </TouchableOpacity>
        <OText style={[styles.title, { fontSize: 18 }]}>{t('MY_ORDERS', 'My orders')}</OText>
        <IconWrapper>
      ...
      <View style={{ flex: 1, minHeight: HEIGHT_SCREEN - 450 }}>
        ...
        >
          ...
          {!internetLoading && !loadingValidation &&
            ((!ordersValidation?.loading &&
              (ordersValidation?.error?.length ||
                ordersValidation?.orders?.length === 0)) ||
              (currentTabSelected === 'logisticOrders' &&
          ...
          >
            <View style={{
              ...
              <ModalTitle style={{
                paddingLeft: 30,
                // fontFamily:'Outfit'
              }}>{t('SEARCH_ORDERS', 'Search orders')}</ModalTitle>
            </View>
