...
export const OrderListGroups = (props) => {
  ...
  const [inProOrders, setInProOrders] = useState(false);
  ...
  const ordersGroupStatus = {
    ...
    pending: orderGroupStatusCustom?.pending ?? [0, 13, 4, 7, 8],
  ...
  const loadInProOrdersforPickupcompleted = async ({ newFetch, newFetchCurrent, isFetchpickup } = {}, options = {}) => {
    if (newFetch) {
      ...
      ordersGroup = {
        ...
        ['inProgress']: {
          ...orderStructure,
          defaultFilter: ordersGroupStatus['inProgress'],
          currentFilter: ordersGroup['inProgress']?.currentFilter
        }
      }
    }
    ...
    try {
      ...
      const { content: { error, result, pagination } } = await getOrders({
        page: 1,
        pageSize,
        orderStatus: options?.allStatusses ? null : ordersGroup[isFetchpickup ? 'inProgress' : currentTabSelected]?.currentFilter,
        newFetch: isFetchpickup ? true : (newFetch || newFetchCurrent)
      })
      const _ordersCleaned = error
        ? (newFetch || newFetchCurrent)
          ? []
          : sortOrders(ordersGroup[isFetchpickup ? 'inProgress' : currentTabSelected]?.orders)
        : (newFetch || newFetchCurrent)
          ? sortOrders(result)
          : sortOrders(ordersGroup[isFetchpickup ? 'inProgress' : currentTabSelected]?.orders.concat(result))
      const hasStatusEightOrNine = _ordersCleaned.some(item => item.status === 8 || item.status === 9);

      return _ordersCleaned
      // if (isFetchpickup) {

      //   setOrdersGroup({
      //     ...ordersGroup,
      //     ['inProgress']: {
      //       ...ordersGroup['inProgress'],
      //       loading: false,
      //       orders: _ordersCleaned,
      //       error: error ? result : null,
      //       fetched: true,
      //       pagination: {
      //         ...ordersGroup['inProgress'].pagination,
      ...
      setInProOrders(false)
    }
  }

  const loadInProOrders = async ({ newFetch, newFetchCurrent } = {}, options = {}) => {
    console.log('load pre order/////', currentTabSelected)
    if (newFetch) {
      ordersStatusArray?.map(tab => {
        ordersGroup = {
          ...ordersGroup,
    ...
    try {
      ...
      const hasStatusEightOrNine = _ordersCleaned.some(item => item.status === 8 || item.status === 9);
      setInProOrders(hasStatusEightOrNine)
    } catch (err) {
      setInProOrders(false)
    }
  }

  const handleToggleAvalaibleStatusDriver = async () => {
    try {
      const response = await ordering
        .users(session?.user?.id)
        .save({ available: true }, { accessToken })
  ...
  const loadOrders = async ({ newFetch, newFetchCurrent, isFetchpickup } = {}, options = {}) => {
    if (!session?.user?.available) {
    ...
    if (newFetch) {
      ...
      setCombineTabsState('inProgress')
      ordersGroup = {
        ...ordersGroup,
        [currentTabSelected]: {
          ...orderStructure,
  ...
  const getStatusById = (id) => {
    ...
    const pending = orderGroupStatusCustom?.pending ?? [0, 13, 4, 7, 8]
  ...
  const handleMarkerPress = (order) => {
    ...
    if (order?.status == 1 || order?.status == 11 || order?.status == 15 || order?.status == 2 || order?.status == 5 || order?.status == 6 || order?.status == 10 || order?.status == 12 || order?.status == 16 || order?.status == 17) {
      setCompletedOrder((prev) => ({ ...prev, ...order }));
  ...
  // oepn map for active order lsit
  ...
  const handleChangeOrderStatus = async (status, orderIds, body = {}) => {
    try {
      ...
      if (status === 6) {
        ...
        const status = getStatusById(result[0]?.status)
        const orderList = ordersGroup[status]?.orders
        const indexToUpdate = orderList?.findIndex((o) => o?.id === result[0]?.id)
        orderList[indexToUpdate] = result[0]
        setOrdersGroup({
          ...ordersGroup,
          [status]: {
            ...ordersGroup[status],
  ...
  useEffect(() => {
    if (!ordersGroup[currentTabSelected]?.fetched && props.isNetConnected) {
      loadInProOrders({ newFetchCurrent: true })
    }
  ...
  useEffect(() => {
    ...
    if (props.order && props.order?.status == 1 || props.order?.status == 11 || props.order?.status == 15 || props.order?.status == 2 || props.order?.status == 5 || props.order?.status == 6 || props.order?.status == 10 || props.order?.status == 12 || props.order?.status == 16 || props.order?.status == 17) {
      handleMarkerPress(props.order)
      setOrdersGroup({
        ...ordersGroup,
        [currentTabSelected]: {
          ...ordersGroup[currentTabSelected],
    ...
    if (props.order && props.order?.status == 0 || props.order?.status == 3 || props.order?.status == 4 || props.order?.status == 7 || props.order?.status == 8 || props.order?.status == 9 || props.order?.status == 13 || props.order?.status == 14 || props.order?.status == 18 || props.order?.status == 19 || props.order?.status == 20 || props.order?.status == 21 || props.order?.status == 22 || props.order?.status == 23 || props.order?.status == 24 || props.order?.status == 25 || props.order?.status == 26) {
      console.log('pending')
    ...
    <>
      {UIComponent && (
        <UIComponent
          ...
          handelCancelFinding={handelCancelFinding}
          selectedOrder={selectedOrder}
          setSelectedOrder={setSelectedOrder}
          handleMarkerPress={handleMarkerPress}
          rejectOrder={rejectOrder}
          completedOrder={completedOrder}
          inProOrders={inProOrders}
          loadorder={loadInProOrders}
          pickupcompleted={loadInProOrdersforPickupcompleted}
        />
      )}
    </>
  )
