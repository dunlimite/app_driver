...
import { useLanguage, useUtils, useConfig, useEvent } from '@components';
import { DeviceOrientationMethods, OText } from '@ui'
...
export const OrderItem = React.memo((props: any) => {
  ...
  const [allowColumns, setAllowColumns] = useState({
    timer: isProjectEnterpricePlan,
    slaBar: isProjectEnterpricePlan,
  })
  ...
  const styles = StyleSheet.create({
    cardButton: {
      flex: 1,
      paddingVertical: (isIpad || isTablet) ? 15 : 10,
      marginBottom: IS_PORTRAIT ? 10 : 0,
      marginLeft: 3,
      marginRight: 3,
      backgroundColor: isProjectEnterpricePlan
    ...
    >
      <Card key={order.id} style={{
        ...
        <Information>
          ...
          >
            {!!order.business?.name && (
              <OText numberOfLines={1} style={[styles.title, { paddingLeft: 5 }]}>
                {order.business?.name}
              </OText>
            )}
          ...
          <View style={{
            ...
            {!order?.order_group_id && showExternalId && !order?.order_group && (
              ...
              >
                ...
                {`${t('NO', 'Order No.')}` + ` ${order?.id}` + ' · ' + `${order?.delivery_datetime_utc
                  ? parseDate(order?.delivery_datetime_utc, { outputFormat: 'MM/DD/YYYY HH:mm A' })
                  : parseDate(order?.delivery_datetime, { utc: false, outputFormat: 'MM/DD/YYYY HH:mm A' })
                  }`}
              </OText>
            )}
