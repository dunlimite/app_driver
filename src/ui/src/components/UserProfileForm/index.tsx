...
import { View, StyleSheet, ScrollView, Text, ActivityIndicator, Pressable,Modal,Image, TouchableOpacity, ImageBackground, Platform, StatusBar } from 'react-native';
  ...
  useEffect(() => {
    ...
    if (userState?.result?.result && !isLoadingDriver) {
      if (userState?.result?.error) {
        const messageError = Array.isArray(userState?.result?.error) ? userState?.result?.error?.[0] : userState?.result?.error;
        showToast(ToastType.Error, messageError?.includes?.('_') ? getTraduction(messageError, t) : messageError);
      } else {
        showToast(
          ToastType.Success,
    ...
    paddingTop:Platform.OS === 'android' ? StatusBar?.currentHeight : 0
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <Text style={{
                    ...
                    fontWeight: '400',
                    marginTop: 10
                  }}>{t('ACCOUNT_ALERT', 'Account alert')}</Text>
                  <Text style={{
                    fontSize: 15,
                    color: '#979B9D',
                    fontFamily: 'Outfit-Regular',
                  ...
                  }}>{t('QUESTION_REMOVE_ACCOUNT', 'Are you sure that you want to remove your account?')}</Text>
                </View>
                <View style={{
                  flexDirection: 'row',
                  ...
                  <TouchableOpacity
                  onPress={() => setremoveaccmodal(false)}
      ...
      <View style={styles.header}>
        ...
        <Text style={{
          textAlign: 'center',
          width: '100%',
          fontSize:  Platform.OS === 'android' ? moderateScale(17) :16,
                ...
                <>
                  {userState.loadingDriver ? (
                    ...
                    <ToggleSwitch
                      ...
                      disabled={userState?.loadingDriver}
                      animationSpeed={200}
                    />
                  )}
                </>
          ...
          */}
          {allowDeleteDriverAccount && (
            <View style={{
              ...
              <RemoveAccount disabled={isAdmin} onPress={() => setremoveaccmodal(!removeAccModal)} activeOpacity={0.7}>
                <AntDesignIcon size={16} name='close' color={theme.colors.textNormal} style={{ marginEnd: 14 }} />
                <OText
                 size={14}
                  lineHeight={24} weight={'400'} style={{ fontSize:15,fontFamily:'Outfit-Regular' }}
                  color='#EE4140'
                  // style={[styles.label,{color:'#EE4140'}]}
                  >{t('REMOVE_ACCOUNT', 'Remove account')}</OText>
              </RemoveAccount>
            </View>
          )}
          <OModal
