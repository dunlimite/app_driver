...
export const OrderContentComponent = (props: OrderContent) => {
  ...
  const [showClientFields, setShowClientFields] = useState<boolean>(true);
  ...
  return (
    <OrderContent isOrderGroup={isOrderGroup} lastOrder={lastOrder}>
      ...
      {order?.metafields?.length > 0 && (
        <OrderBusiness>
          <TouchableOpacity onPress={() => setShowCustomFields((prev: boolean) => !prev)} style={styles.block}>
      ...
      <OrderBusiness>
        <TouchableOpacity onPress={() => setShowBusinessFields((prev: boolean) => !prev)} style={styles.block}>
          <OText style={{ marginBottom: 5 }} size={16} weight="600">
            {t('BUSINESS_DETAILS', 'Business details')}
          </OText>
          <AntDesignIcon
            name={showBusinessFields ? 'up' : 'down'}
            size={14}
          />
        </TouchableOpacity>
        {showBusinessFields && (
      ...
      <OrderCustomer>
        <TouchableOpacity onPress={() => setShowClientFields((prev: boolean) => !prev)} style={styles.block}>
          <OText style={{ marginBottom: 5 }} size={16} weight="600">
            {t('CUSTOMER_DETAILS', 'Customer details')}
          </OText>
          <AntDesignIcon
            name={showClientFields ? 'up' : 'down'}
            size={14}
          />
        </TouchableOpacity>
        {showClientFields && (
