...
import { CouponControl } from 'ordering-ui-native-release/themes/original';
import { OInput, OModal, OText, OAlert } from 'ordering-ui-native-release/themes/original';
import { verifyDecimals } from '../../utils_ch';
import AntIcon from 'react-native-vector-icons/AntDesign'
import { TaxInformation } from 'ordering-ui-native-release/themes/original';
import { TouchableOpacity } from 'react-native';
import { MomentOption } from 'ordering-ui-native-release/themes/original';
        ...
        <>
          ...
          {cart?.valid && (
            <OSBill>
              ...
              {orderState?.options?.type === 1 && !hideDeliveryFee && !cart?.extra_value_courier_price && (
                <OSTable>
			  ...
			  {orderState?.options?.type === 1 && !hideDeliveryFee && cart?.extra_value_courier_price > 0 && (
                <OSTable>
                  <OText size={12}>{t('DELIVERY_FEE', 'Delivery Fee')}</OText>
                  <OText size={12}>{parsePrice(cart?.extra_value_courier_price + getIncludedTaxes(true))}</OText>
                </OSTable>
              )}
              {
              ...
              {cart?.business_id && cart?.status !== 2 && !hideCartComments && (
                <OSTable>
                  <View style={{ width: '100%', marginTop: 20 }}>
                    <OText size={12}>{t('INSTRUCTIONS_DRIVER', 'Instructions for the driver')}</OText>
                    <View style={{ flex: 1, width: '100%' }}>
                      <OInput
                        value={cart?.comment}
                        placeholder={t('INSTRUCTIONS_DRIVER_MSG', 'The amount cannot exceed $60')}
