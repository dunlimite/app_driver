...
import { Checkout as CheckoutController } from '../CheckoutController';
import {
	useOrder,
	useSession,
	useApi,
...
import { OText, OIcon, OModal, OButton } from 'ordering-ui-native-release/themes/original';
import  OModalCourier  from '../shared/OModalCourier'
import { AddressDetails } from 'ordering-ui-native-release/themes/original';
import { DeliveryAddressDetails } from '../DeliveryAddressDetails';
import { PaymentOptions } from '../PaymentOptions';
import { DriverTips } from 'ordering-ui-native-release/themes/original';
import { NotFoundSource } from 'ordering-ui-native-release/themes/original';
import { UserDetails } from 'ordering-ui-native-release/themes/original';
import { PaymentOptionWallet } from 'ordering-ui-native-release/themes/original';
import { PlaceSpot } from 'ordering-ui-native-release/themes/original'
import { SignupForm } from 'ordering-ui-native-release/themes/original'
import { LoginForm } from 'ordering-ui-native-release/themes/original'
...
import { FloatingButton } from 'ordering-ui-native-release/themes/original';
import { Container } from 'ordering-ui-native-release/themes/original';
import NavBar from '../NavBar';
import { OrderSummary } from '../OrderSummary';
import { getTypesText } from '../../utils_ch';
import { CartStoresListing } from 'ordering-ui-native-release/themes/original';
import { PaymentOptionsWebView } from '../PaymentOptionsWebView';
import { DeviceOrientationMethods } from '../../hooks/DeviceOrientation'
import { PaymentOptionsWebViewPesaPal } from '../PaymentOptionsWebViewPesaPal';
import { PaymentOptionsWebViewGreenpayV2 } from '../PaymentOptionsWebViewGreenpayV2';
...
const CheckoutUI = (props: any) => {
	const {
		...
		newUUID,
		openGreenPayCS,
		navigateToGreenPayCS,
		setLoadingCustom,
		loadingCustom,
		customLoadingCtrl,
		addressState,
		loadAddress,
		placeOrderGreenPayV2card
	} = props
	...
	const [paymentUsaCardData, setPaymentUsaCardData] = useState('')
	const [placeOrderpaymentUSA, setPlaceOrderpaymentUSA] = useState(false)
	const [navigateToGreenPay, setNavigateToGreenPay] = useState('')
	const [openGreenPay, setOpenGreenPay] = useState(false)
	...
	const isDisabledButtonPlace = loading || !cart?.valid || (!paymethodSelected && cart?.balance > 0) ||
		...
		validateCouponField ||
		validateZipcodeCard || 
		(businessDetails?.business?.courierbus === true && (!addressState?.address?.deliveryaddress || addressState?.address?.deliveryaddress == ""))
	...
	const [uncheckPaymentOption, setUncheckPaymentOption] = useState(false)
	const [showPaymentGatewayPage, setShowPaymentGatewayPage] = useState<any>({closedByUsed: false, open: false});
	const [openGreenpayCardSave, setOpenGreenpayCardSave] = useState(false)
	const [openGreenpayCardSaveUrl, setOpenGreenpayCardSaveUrl] = useState('')
	const [placeOrderpaymentGreenPay, setPlaceOrderpaymentGreenPay] = useState(false)
	const onNavigationStateChange = (webViewState: { url: string; }) => { 
		if(webViewState){
			console.log('webViewState')
			var rawURL = webViewState.url;
			console.log(rawURL)

			//For card tokenization 
			if(rawURL.indexOf("card_token=") > -1){
				const card_token_str = rawURL.split('card_token=')
				if(card_token_str[1] == '0'){
					console.log('Card Token generation failed');
					setLoadingCustom(false)
					setUncheckPaymentOption(true)
					setShowPaymentGatewayPage({closedByUser: true, open: false})
					setShowGateway({ closedByUser: false, open: false })
					
					setOpenGreenpayCardSave(false)
					showToast(ToastType.Error, t('CARD_TOKEN_GENERATION_FAILED', 'Card Token generation failed.'))	
				}
				if(card_token_str[1] == '1'){
					console.log('Card added successfully, please choose a card and go for payment');
					setLoadingCustom(false)
					setUncheckPaymentOption(true)	
					setShowPaymentGatewayPage({closedByUser: true, open: false})
					setShowGateway({ closedByUser: false, open: false })
					setOpenGreenpayCardSave(false)
					showToast(ToastType.Success,  t('CARD_ADDED_SUCCESSFULLY_PLEASE_CHOOSE_A_NEW_CARD_AND_GO_FOR_PAYMENT', 'Card added successfully, please choose a card and go for payment.'))
				}
			}//end if

			//For payment with tokenize card
			if(rawURL.indexOf("payment_status=") > -1){
				console.log('checking 1.1..')
				const new_uuid_str = rawURL.split('new_uuid=')
				console.log('checking 1.2..')
				if(new_uuid_str[1] == ''){
					console.log('checking 1.3..')
					setLoadingCustom(false)
					setUncheckPaymentOption(true)
					setShowPaymentGatewayPage({closedByUser: true, open: false})
					setShowGateway({ closedByUser: false, open: false })					
					showToast(ToastType.Error, t('SORRY_PAYMENT_FAILED', 'Sorry! payment Failed'))
				}//end if
				...
				if(new_uuid_str[1] != ''){
					...
					setShowPaymentGatewayPage({closedByUser: true, open: false})
					setShowGateway({ closedByUser: false, open: false })
					console.log('new uuid: ' + new_uuid_str[1])
					onNavigationRedirect('OrderDetails', { orderId: new_uuid_str[1], isFromCheckout: true })
				}//end if
			}//end if
		}	 
	}
	
	const handleCancelAddressPop = () => {
		setShowPickupAddress(false)
	  }
	  const handleRedirectAddressPop = () => {
		setShowPickupAddress(false)
	  }
	const handleOpenAddressPop = () => {
	...
	const handlePlaceOrder = async(confirmPayment: any, forcePlace: boolean = false) => {

		// if(paymethodSelected?.gateway === 'greenpay'){
		// 	if (!userErrors.length && (!requiredFields?.length || (allowedGuest && (paymethodSelected?.gateway === 'greenpay')))) { 

		// 		if(paymethodSelected?.data?.id == undefined){
		// 			showToast(ToastType.Error, t('PLEASE_ADD_CARD_OR_SELECT_A_CARD', 'Please add card or select a card'))
		// 		}else{	
		// 			//setCustomLoading(true)
		// 			const curOrderCode = cart?.business_id+generateTransactionId().toUpperCase();
		// 			console.log('checkout page card id: ' + paymethodSelected?.data?.id)
		// 			const curr_url = 'https://plugins-development.ordering.co/vanvidelivery/greenpay/';
		...
		// 				showToast(ToastType.Success, t('SESSION_GENERATION_SUCCESS', 'Session generation success')) 
		// 				const body = {}
		// 				body.green_p_session = contentSession.session
		// 				body.green_p_token = contentSession.token
		// 				body.green_p_code = curOrderCode
						
		// 				handlerClickPlaceOrder && handlerClickPlaceOrder(null, body)
		// 			}else{
		// 				//setCustomLoading(false)
		// 				showToast(ToastType.Error, t('SESSION_GENERATION_FAILED', 'Session generation failed'))             
		// 			}//end if 
		// 			//end service call
		// 		}//end if
		// 	}
	...
	useEffect(() => {
		loadAddress(options?.user_id, options?.address_id)
	...
	useEffect(() => {
		if(businessDetails?.business?.courierbus === true && (!addressState?.address?.deliveryaddress || addressState?.address?.deliveryaddress == "")){
			showToast(ToastType.Error, t('ADD_DELIVERY_ADDRESS', 'Please Add delivery address'));
		}
	}, [businessDetails?.business?.courierbus, addressState?.address?.deliveryaddress])
	useEffect(() => {
		if (errors) {
	...
	useEffect(() => {
		console.log('closedByUser:' + showGateway.closedByUser + ' gateway name: ' + paymethodSelected?.gateway)
		if (paymethodSelected?.gateway == 'greenpay') {
			if(showGateway.closedByUser == true){
				setLoadingCustom(false)
				setUncheckPaymentOption(true)
				//setLoadingCustom(false)
				showToast(ToastType.Error, t('SORRY_PAYMENT_CANCELED', 'Sorry! payment canceled'));
			}
		}else{
			//onFailPaypal()
		}
	...
	useEffect(()=>{
		if(redirectToConfirm == true){
			console.log('redirectToConfirm: ' + redirectToConfirm + ' newUUID: ' +newUUID )
            onNavigationRedirect('OrderDetails', { orderId: newUUID, isFromCheckout: true })
		}
	},[redirectToConfirm])
	...
	//Add new green pay card
	const addGreenPayCard = async () => {		
	  try {
		const curr_url = 'https://plugins-development.ordering.co/vanvidelivery/greenpay/';
		const curOrderCode = cart?.business_id + generateTransactionId().toUpperCase();
		const total1 = cart?.balance ?? cart?.total
		...
		const functionFetch = `https://plugins-development.ordering.co/vanvidelivery/greenpay/card_session.php`
		const response = await fetch(functionFetch, requestOptions)
		const content = await response.json()
		console.log('API response: ' + JSON.stringify(content));
		console.log('status: ' + content.status + ' sandbox: ' + content.sandbox)
		setPlaceOrderpaymentUSA(false)
		setLoadingCustom(true)	
		if(content.status == true){
			if(content.sandbox == true){
				const redirectToGreenpay = "https://sandbox-tokenizeform.greenpay.me/"+ content.session;
				console.log('go to: ' + redirectToGreenpay)	
				setNavigateToGreenPay(redirectToGreenpay)		
				setOpenGreenPay(true)				
			}else{
				const redirectToGreenpay = "https://tokenizeform.greenpay.me/"+ content.session;	
				console.log('go to: ' + redirectToGreenpay)	
				setNavigateToGreenPay(redirectToGreenpay)	
				setOpenGreenPay(true)		
			} 	
				
			setShowPaymentGatewayPage({closedByUser: false, open: true})
			setShowGateway({ closedByUser: false, open: true })	
		}else{
			showToast(ToastType.Error, t('SESSION_GENERATION_FAILED', 'Session generation failed'))
			setUncheckPaymentOption(true)
	...
	//Add new green pay card
	const addGreenPayVVCard = async () => {	
		const user_id = cart.user_id
		const business_id =  cart?.business_id	
	  	const curOrderCodeaddcard = cart?.business_id + generateTransactionId().toUpperCase();
	  	const redirectToGreenpay = "https://plugins-development.ordering.co/vanvidelivery/greenpay_v2_dev/create_tokenize_order.php?business_id="+business_id+"&user_id="+user_id+"&code="+curOrderCodeaddcard;
	  	setOpenGreenpayCardSaveUrl(redirectToGreenpay)
	  	setOpenGreenpayCardSave(true)
		setPlaceOrderpaymentGreenPay(false)
	  
  	}//end fun

	useEffect(()=>{
		console.log('placeOrderpaymentUSA:' + placeOrderpaymentUSA)
		if(placeOrderpaymentUSA == true){
			console.log('create a new session and redirect to add card page')
			setLoadingCustom(true)
			addGreenPayCard()
		}
	},[placeOrderpaymentUSA])
	
	useEffect(()=>{
		console.log('placeOrderpaymentGreenPay:' + placeOrderpaymentGreenPay)
		if(placeOrderpaymentGreenPay == true){
			console.log('add card page')
			setLoadingCustom(true)
			addGreenPayVVCard()
		}
	},[placeOrderpaymentGreenPay])
	useEffect(()=>{
		console.log('placeOrderGreenPayV2card:' + placeOrderGreenPayV2card)
		if(placeOrderGreenPayV2card===true){
			console.log('testredirect:' + placeOrderGreenPayV2card)
			onNavigationRedirect('OrderDetails', { orderId: cart?.uuid, isFromCheckout: true })
		}
	},[placeOrderGreenPayV2card]) 

	useEffect(()=>{
		console.log('openGreenpayCardSave:' + openGreenpayCardSave + ' navigateToGreenPayCS: ' + navigateToGreenPayCS)
		if(openGreenpayCardSave == true){
			setNavigateToGreenPay('https://www.google.com/')
			setLoadingCustom(true)				
			setShowPaymentGatewayPage({closedByUser: false, open: true})
			setShowGateway({ closedByUser: false, open: true })	
		}
	},[openGreenpayCardSave])	


	useEffect(()=>{
		console.log('openGreenPayCS:' + openGreenPayCS + ' navigateToGreenPayCS: ' + navigateToGreenPayCS)
		if(openGreenPayCS == true){
			setNavigateToGreenPay(navigateToGreenPayCS)		
			setOpenGreenPay(true)
			setLoadingCustom(true)				
			setShowPaymentGatewayPage({closedByUser: false, open: true})
			setShowGateway({ closedByUser: false, open: true })	
	...
	return (
		<>
		<View>
		{customLoadingCtrl && (<Spinner visible={customLoadingCtrl} />)}
		</View>
			...
			<Container pt={0} forwardRef={containerRef} showsVerticalScrollIndicator={false} noPadding onScroll={handleScroll}>
				...
				<ChContainer style={styles.pagePadding}>
					...
					{!isGiftCardCart && !hideBusinessMap && !businessDetails?.business?.courierbus && (
						<ChSection>
							<ChAddress>
								{(businessDetails?.loading || cartState.loading) ? (
									...
									<AddressDetails
										...
										HeaderTitle={<HeaderTitle text={t('PICKUP_ADDRESS', 'Pickup address')} mb={0} />}
									/>
								)}
							</ChAddress>
					...
					{/*{!isGiftCardCart && !hideBusinessMap && businessDetails?.business?.courierbus && addressState?.address?.deliverylocation && (
						<ChSection>
							<ChAddress>
								{(businessDetails?.loading || cartState.loading) ? (
									<Placeholder Animation={Fade}>
									...
									<DeliveryAddressDetails
										cart={cart}
										navigation={navigation}
										location={addressState?.address?.deliverylocation}
										businessLogo={businessDetails?.business?.logo}
										isCartPending={cart?.status === 2}
										uuid={cartUuid}
										apiKey={configs?.google_maps_api_key?.value}
										...
										handleOpenAddressPop={handleOpenAddressPop}
						...
						<ChSection>
							<ChPaymethods>
								...
								<PaymentOptions
									...
									uncheckPaymentOption={uncheckPaymentOption}
									setPlaceOrderpaymentUSA={setPlaceOrderpaymentUSA}
									setPaymentUsaCardData={setPaymentUsaCardData}
									setPlaceOrderpaymentGreenPay={setPlaceOrderpaymentGreenPay}
								/>
							</ChPaymethods>
						</ChSection>
					)}
								...
								{validateCouponField &&
									...
									<>
										<OText
											...
											size={12}
										>
											{t('ADD_DELIVERY_ADDRESS', 'Please Add delivery address')}
										</OText>
									</>
								)}
							</ChErrors>
					...
					<OModalCourier
					open={showPickupAddress}
					entireModal
				  >
				  <DeliveryAddressForm
				  handleCancelAddressPop={handleCancelAddressPop}
				  handleRedirectAddressPop={handleRedirectAddressPop}
				  deliveryaddressState={addressState}
				  />
			...
			{!cartState.loading && cart && cart?.status !== 2   && !loadingCustom && (
				<FloatingButton
					handleClick={
			...
			{cart?.paymethod_data?.gateway === 'greenpay' && showPaymentGatewayPage.open && showGateway.open && openGreenPay && (
			   <PaymentOptionsWebViewPesaPal
				   onNavigationRedirect={onNavigationRedirect}
				   onNavigationStateChange={onNavigationStateChange}
				   uri={navigateToGreenPay}
				   user={user}
				   token={token}
				   cart={cart}
				   currency={currency}
		   ...
		   {/* {cart?.paymethod_data?.gateway === 'greenpay_v2' && showPaymentGatewayPage.open && showGateway.open && openGreenPay && (
			   <PaymentOptionsWebViewGreenpayV2
				   onNavigationRedirect={onNavigationRedirect}
				   onNavigationStateChange={onNavigationStateChange}
				   uri={navigateToGreenPay}
				   user={user}
		   ...
		   {openGreenpayCardSave && (
			   <PaymentOptionsWebViewGreenpayV2
				   onNavigationRedirect={onNavigationRedirect}
				   onNavigationStateChange={onNavigationStateChange}
				   uri={openGreenpayCardSaveUrl}
				   setShowGateway={setShowGateway}
				   locationId={'L1NGAY5M6KJRX'}
				   setLoadingCustom={setLoadingCustom}
				   setOpenGreenpayCardSave = {setOpenGreenpayCardSave}
			   />
		   )}
		   {cart?.paymethod_data?.gateway === 'greenpay_v2' && showPaymentGatewayPage.open && showGateway.open && openGreenPay && (
			   <PaymentOptionsWebViewGreenpayV2
		   ...
		   {cart?.paymethod_data?.gateway === 'greenpay_plugin' && showPaymentGatewayPage.open && showGateway.open && openGreenPay && (
			   <PaymentOptionsWebViewGreenpayV2
				   onNavigationRedirect={onNavigationRedirect}
				   onNavigationStateChange={onNavigationStateChange}
				   uri={navigateToGreenPay}
