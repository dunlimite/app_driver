...
export const AddressDetails = (props) => {
  ...
  const formatUrlMethod = (businessLocation) => {
    const orderLocation = addressState?.address?.deliverylocation
	//alert("lat=>"+orderLocation?.lat)
	//alert("lat=>"+orderLocation?.lng)
    ...
    const staticmapUrl = isMultiCheckout
      ...
      : `https://maps.googleapis.com/maps/api/staticmap?size=${mapConfigs?.mapSize?.width || 500}x${mapConfigs?.mapSize?.height || 190}&scale=2&maptype=roadmap&markers=color:red%7C${orderLocation?.lat},${orderLocation?.lng}&key=${GM_API_KEY}`
  ...
  }, [logo, JSON.stringify(location), JSON.stringify(addressState?.address?.deliverylocation), JSON.stringify(orderState?.options?.address?.address)])
