...
export const BusinessSearchList = (props) => {
  ...
  const handleSearchbusinessAndProducts = async (newFetch, options, val) => {
    try {
      ...
      const updatedArray = result.filter(business => business.id !== 4638)
      if (error) {
        setBusinessesSearchList({
          businesses: [],
          loading: false,
      ...
      if (pagination.current_page !== pagination.total_pages) {
        const remainingItems = pagination.total - updatedArray.length
      ...
      setBusinessesSearchList({
        ...businessesSearchList,
        businesses: cityId ? (newFetch ? updatedArray : [...businessesSearchList?.businesses, ...updatedArray])?.filter(_business => _business?.city_id === cityId) : newFetch ? updatedArray : [...businessesSearchList?.businesses, ...updatedArray],
        loading: false,
        lengthError: false
      })
    } catch (err) {
