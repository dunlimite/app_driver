...
export const PageBanner = (props) => {
  ...
  const [sliInterval, setSliInterval] = useState(5000)
  ...
  const handleGetPageBanner = async () => {
    ...
    try {
	    let res = await fetch('https://plugins-development.ordering.co/vanvidelivery/get_slider_settings.php', {
		  method: 'post',
		  headers: {
	  ...
	  let resultData = await res.json(); 
		setSliInterval(resultData.result.durationinsec)
