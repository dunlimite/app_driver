...
import { useUtils, useEvent } from 'ordering-components-external/native'
import { PageBanner as PageBannerController } from './PageBanner'
...
const PageBannerUI = (props: any) => {
  ...
  useEffect(() => {
    if (!pageBannerState.banner?.items || pageBannerState.banner.items.length === 0) return;

    const startAutoPlay = () => {
      intervalRef.current = setInterval(() => {
        const nextIndex = (currentIndex + 1) % pageBannerState.banner.items.length;
  ...
  }, [pageBannerState.banner?.items, currentIndex, interval]);
