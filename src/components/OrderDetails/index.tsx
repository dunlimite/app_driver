...
import { ProductItemAccordion } from 'ordering-ui-native-release/themes/original';
import { TouchableOpacity } from 'react-native-gesture-handler';
import { OrderDetailsParams } from '../../types';
import { GoogleMap } from 'ordering-ui-native-release/themes/original';
import { verifyDecimals, getOrderStatus, getOrderStatuPickUp } from '../../utils_ch';
import { OSRow } from '../OrderSummary/styles';
import AntIcon from 'react-native-vector-icons/AntDesign'
import { TaxInformation } from 'ordering-ui-native-release/themes/original';
              ...
              <OrderCustomer>
                ...
                <Customer>
                  <InfoBlock>
					...
					{!hideCustomerAddress && order?.deliveryaddress && (
                      <OText
                        size={12}
                        lineHeight={18}
                        color={theme.colors.textNormal}
                        mBottom={2}>
                        {t('PICKUP_ADDRESS', 'Pickup Address')}: {order?.deliveryaddress}
                      </OText>
                    )}
                    {!hideCustomerAddress && (
                      <OText
                        ...
                        color={theme.colors.textNormal}
                        mBottom={2}>
                        {t('DELIVERY_ADDRESS', 'Delivery Address')}: {order?.customer?.address}
                      </OText>
                    )}					
					{order?.pickupdate && (
                      <OText
                        ...
                        color={theme.colors.textNormal}
                        mBottom={2}>
                        {t('PICKUP_DATE', 'Pickup Date')}: {order?.pickupdate} {order?.pickuptime}
                      </OText>
                    )}
					{order?.courier_additional_comments && (
                      <OText
                        size={12}
                        lineHeight={18}
                        color={theme.colors.textNormal}
                        mBottom={2}>
                        {t('ADDITIONAL_COMMENTS', 'Additional Comments')}: {order?.courier_additional_comments}
                      </OText>
                    )}
					{order?.courier_instru_driver && (
                      <OText
                        size={12}
                        lineHeight={18}
                        color={theme.colors.textNormal}
                        mBottom={2}>
                        {t('INSTRUCTIONS_DRIVER', 'Instructions for driver')}: {order?.courier_instru_driver}
                      </OText>
                    )}
