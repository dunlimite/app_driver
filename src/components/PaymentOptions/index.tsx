...
import { PaymentOptionCash } from 'ordering-ui-native-release/themes/original';
import { StripeElementsForm } from 'ordering-ui-native-release/themes/original';
import { StripeCardsList } from 'ordering-ui-native-release/themes/original';
import { PaymentOptionCard } from '../PaymentOptionCard'
import { PaymentUSACardsList } from '../PaymentUSACardsList';
import { PaymentGreenPayV2Cardlist } from '../PaymentGreenPayV2Cardlist';
...
import { getIconCard, flatArray } from '../../utils_ch';
...
const PaymentOptionsUI = (props: any) => {
	const {
		...
		setPaymethodClicked,
		androidAppId,
		setPlaceOrderpaymentUSA,
		setPaymentUsaCardData,
		uncheckPaymentOption,
		setPaymentGreenPayVCardData,
		setPlaceOrderpaymentGreenPay,
		businessId
	} = props
	...
	const [addCardOpen, setAddCardOpen] = useState({ stripe: false, stripeConnect: false, card: false, authorizenet: false });
	const paymethodSelected = props.paySelected || props.paymethodSelected || isOpenMethod?.paymethod
	const [onFACInputForm, setOnFACInputForm] = useState(false)
	...
	useEffect(()=>{
		console.log('uncheckPaymentOption: ' + uncheckPaymentOption)
		if(uncheckPaymentOption == true){
			handlePaymethodClick(null)
		}
	},[uncheckPaymentOption])
      ...
      >
        {paymethodSelected?.gateway === 'paypal' && (
          ...
          />
				...
				{/* greenpay PG */}
				{isOpenMethod?.paymethod?.gateway === 'greenpay'  && (
					<View>
						{myLoading && (<Spinner visible={myLoading} />)}
						<OButton
							text={t('ADD_PAYMENT_CARD', 'Add New Payment Card')}
							bgColor={theme.colors.white}
							...
							textStyle={{ color: theme.colors.primary, fontSize: 12 }}
							imgRightSrc={null}					
							onClick={() => setPlaceOrderpaymentUSA(true)}
						/>
						<PaymentUSACardsList
							paymethod={isOpenMethod?.paymethod}
							businessId={props.businessId}
							...
							onSelectCard={handlePaymethodDataChange}
							onNavigationRedirect={onNavigationRedirect}
							onCancel={() => setAddCardOpen({ ...addCardOpen, authorizenet: false })}
						/>
					</View>
				)}
				{/*{isOpenMethod?.paymethod?.gateway === 'greenpay_v2'  && (
					<View>
						{myLoading && (<Spinner visible={myLoading} />)}
						<OButton
							text={t('ADD_PAYMENT_CARD', 'Add New Payment Card')}
							...
							textStyle={{ color: theme.colors.primary, fontSize: 12 }}
							imgRightSrc={null}					
							onClick={() => setPlaceOrderpaymentGreenPay(true)}
						/>
						<PaymentGreenPayV2Cardlist
							paymethod={isOpenMethod?.paymethod}
